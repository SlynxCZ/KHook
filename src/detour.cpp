...
#define WIN_ONLY(x)
    ...
    static FUNCTION_ATTRIBUTE_PREFIX(void) RecursiveLockUnlockShared(std::shared_mutex* mutex, bool lock)
    FUNCTION_ATTRIBUTE_SUFFIX {
    ...
    KHOOK_API void SaveReturnValue(KHook::Action action, void* ptr_to_return, std::size_t return_size, void* init_op,
                                   void* delete_op, bool original)
    {
        auto loop = g_saved_params.top();
        if (original)
        {
    ...
    KHOOK_API void* DoRecall(KHook::Action action, void* ptr_to_return, std::size_t return_size, void* init_op,
                             void* delete_op)
    {
        g_is_in_recall = true;
    ...
    void copy_stack(DetourCapsule::AsmJit& jit, std::int32_t offset, std::int32_t stack_size,
                    std::int32_t realign_size = 0)
    {
#ifdef KHOOK_X64
        jit.push(rax);
        LINUX_ONLY(jit.push(rdi));
        WIN_ONLY(jit.push(rcx));
        LINUX_ONLY(jit.push(rsi));
        WIN_ONLY(jit.push(rdx));
        LINUX_ONLY(jit.push(rdx));
        WIN_ONLY(jit.push(r8));
        ...
        LINUX_ONLY(jit.pop(rdx));
        WIN_ONLY(jit.pop(r8));
        LINUX_ONLY(jit.pop(rsi));
        WIN_ONLY(jit.pop(rdx));
        LINUX_ONLY(jit.pop(rdi));
        WIN_ONLY(jit.pop(rcx));
        jit.pop(rax);
#else
	if (realign_size != 0) {
        ...
        _stack_size(STACK_SAFETY_BUFFER)
    {
        ...
        static auto begin_detour = [](DetourCapsule::AsmJit& jit, std::uint32_t offset_to_loop_params,
                                      std::uint32_t offset_to_regs, std::uint32_t offset_to_stack,
                                      std::int32_t stack_size, DetourCapsule* capsule)
        {
        ...
        static auto perform_loop = [](DetourCapsule::AsmJit& jit,
                                      std::uintptr_t jit_func_ptr,
                                      std::int32_t func_param_stack_size,
                                      std::int32_t offset_fn_callback,
                                      std::int32_t offset_next_it,
                                      std::int32_t offset_loop_condition,
                                      std::int32_t offset_fn_callback_dynamic)
        {
            auto entry_loop = (std::int32_t)jit.get_outputpos();

            jit.mov(r8, rax(offset_fn_callback_dynamic)); // LinkedList::fn_make_pre_dynamic / fn_make_post_dynamic
            jit.test(r8, r8);
            std::int32_t exit_loop = 0;
            std::int32_t exit_loop_recall = 0;
            jit.jnz(INT32_MAX);
            auto has_dynamic = jit.get_outputpos();
            {
                jit.mov(rcx, rbp);
                jit.call(r8);
                jit.jump(INT32_MAX);
                auto skip_typed = jit.get_outputpos();

                // Typed branch
                jit.mov(r8, rax(offset_fn_callback));
                jit.test(r8, r8);
                jit.jz(INT32_MAX);
                auto exit_loop = jit.get_outputpos();
                {
                    // MAKE PRE/POST CALL (typed)
                    jit.push(r8);
                    jit.push(r8);
                    ...
                    jit.test(rax, rax);
                    jit.jnz(INT32_MAX);
                    exit_loop_recall = jit.get_outputpos();
                    jit.mov(rax, rbp(offsetof(AsmLoopDetails, linked_list_it)));
                    jit.mov(rax, rax(offset_next_it));
                    jit.mov(rbp(offsetof(AsmLoopDetails, linked_list_it)), rax);
                    jit.test(rax, rax);
                    jit.jnz(INT32_MAX);
                    jit.rewrite<std::int32_t>(jit.get_outputpos() - sizeof(std::int32_t),
                                              entry_loop - (std::int32_t)jit.get_outputpos());
                }
                jit.rewrite<std::int32_t>(exit_loop - sizeof(std::int32_t), jit.get_outputpos() - exit_loop);
                jit.rewrite<std::int32_t>(exit_loop_recall - sizeof(std::int32_t),
                                          jit.get_outputpos() - exit_loop_recall);
                jit.mov(rbp(offset_loop_condition), true);

                jit.rewrite<std::int32_t>(skip_typed - sizeof(std::int32_t), jit.get_outputpos() - skip_typed);
            }
            jit.rewrite<std::int32_t>(has_dynamic - sizeof(std::int32_t), jit.get_outputpos() - has_dynamic);
        };
        ...
        _jit.jnz(INT32_MAX);
        {
            auto jnz = _jit.get_outputpos();
            {
                ...
                perform_loop(_jit,
                             reinterpret_cast<std::uintptr_t>(&_jit_func_ptr),
                             func_param_stack_size,
                             offsetof(LinkedList, fn_make_pre),
                             offsetof(LinkedList, next),
                             offsetof(AsmLoopDetails, pre_loop_over),
                             offsetof(LinkedList, fn_make_pre_dynamic));
            }
            _jit.rewrite<std::int32_t>(jnz - sizeof(std::int32_t), _jit.get_outputpos() - jnz);
        }
        ...
        _jit.jnz(INT32_MAX);
        {
            auto jnz = _jit.get_outputpos();
            {
                ...
                _jit.rewrite<std::int32_t>(if_not_supersede - sizeof(std::int32_t),
                                           _jit.get_outputpos() - if_not_supersede);
            }
            _jit.rewrite<std::int32_t>(jnz - sizeof(std::int32_t), _jit.get_outputpos() - jnz);
        }
        ...
        _jit.jnz(INT32_MAX);
        {
            auto jnz = _jit.get_outputpos();
            {
                ...
                perform_loop(_jit,
                             reinterpret_cast<std::uintptr_t>(&_jit_func_ptr),
                             func_param_stack_size,
                             offsetof(LinkedList, fn_make_post),
                             offsetof(LinkedList, prev),
                             offsetof(AsmLoopDetails, post_loop_over),
                             offsetof(LinkedList, fn_make_post_dynamic));
            }
            _jit.rewrite<std::int32_t>(jnz - sizeof(std::int32_t), _jit.get_outputpos() - jnz);
        }
    ...
    static auto perform_loop = [](DetourCapsule::AsmJit& jit,
    	...
    	jit.mov(ecx, eax(offset_fn_callback_dynamic)); // LinkedList::fn_make_pre_dynamic
    	jit.test(ecx, ecx);
    	std::int32_t exit_loop_recall = 0;
    	jit.jnz(INT32_MAX); auto has_dynamic = jit.get_outputpos(); {
    		jit.mov(eax, ebp); // param = AsmLoopDetails*
    		jit.push(eax);
    		jit.call(ecx);
    		jit.add(esp, 4);
    		jit.jump(INT32_MAX);
